...
**Affected versions.** Every release from **1.0.0** (the first release; it has no git tag, its `CHANGELOG.md` heading is dated 2026-06-09) through **v3.20.1**. Because 1.0.0 is untagged, the exact commit a 1.0.0 user has cannot be identified: 1.0.0 builds from 2026-06-09 on carry the unsafe call sites and are affected, while 1.0.0 commits from before that date do not. The range was derived by walking versions, not tags: `.claude-plugin/plugin.json` history (77 versions), `CHANGELOG.md` headings (76) and git tags (76) agree, and 1.0.0 is the only version with no tag. Each per-site first version below is the first version whose tree contains that call shape, checked at the previous release too. The defects were measured on the earliest tree where each was executed (WSL Ubuntu, Node 24.19.0, a fake `HOME`, hooks under a 3 GB address-space cap and a 30 s timeout): the newest 1.0.0 tree (`90d4b12`), v2.0.0, v3.0.0 and v3.3.0. **Fixed in:** the repository, commit `a4f0cff`; that commit is not in a release yet, so the next release after v3.20.1 ships it, and this entry will name that version when it does. Until then, do not run a CoalMine hook, `install.mjs` or `configure.mjs` in a clone you do not trust.
...
| **First affected** | The platform rules file create and append: **1.0.0** (append measured on the newest 1.0.0 tree, v2.0.0 and v3.20.1); its rewrite on uninstall: **v2.4.0**. The git hook write: **1.0.0** (by source); its backup copy: **v2.2.1**. The skills directory install: **1.0.0** (by source). The manifest write: **v2.6.0**. The default project config copy: **v3.7.0**. |
