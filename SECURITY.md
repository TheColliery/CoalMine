...
- A stray skills tree in the directory a project-level agent folder pointed at. `install.mjs` writes CoalMine's skill folders into the project's `.github/skills`, `.agents/skills` or the like; if `.github`, `.agents` (or `.gemini`, `.cursor`, `.windsurf`) in that clone was a link to a directory elsewhere, the whole skills tree landed in the link's target. Look for a `skills` directory of CoalMine skill folders (each holding a `SKILL.md`) there, and remove what you did not mean to put there. Measured on 1.0.0 trees from `2aeb8de` (2026-06-05); this write-through applies to every release through v3.20.1.
