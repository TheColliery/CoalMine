...
| | |
|---|---|
| **What happens** | With the Copilot instructions file (`copilot-instructions.md`, in the project's `.github` directory) linked to `~/.bashrc`, `install.mjs copilot` appended CoalMine's own rules block, between `COALMINE:START` and `COALMINE:END` markers, to the shell rc and reported success (measured on v2.0.0 and v3.20.1). The attacker does not choose the appended text; it is CoalMine's template, but it lands in a file outside the project. The same write-through applied to an existing git hook (its bytes are replaced, and a hook that is a link had its target's bytes copied into the `.pre-coalmine` backup slot), the default project config, the manifest, and a link on any project-level agent folder `install.mjs` writes into, which carried the skills install and the default config outside the project. As of v3.20.1 that is nine folders (derived from `scripts/lib/targets.mjs` at `59ee1e7`): `.github`, `.agents`, `.claude`, `.gemini`, `.cursor`, `.windsurf`, `.junie`, `.kiro` and `.augment`; earlier releases target a subset. Measured on v3.20.1 with `.claude`, `.agents` and `.gemini` each linked to an outside directory: the skills tree, its manifest and the default config landed there (and, for `.agents` and `.gemini`, the trigger rules file). |
...
- A stray CoalMine install in the directory a project-level agent folder pointed at. `install.mjs` writes CoalMine's skill folders, and a default config, into the project's agent folder; if any of `.github`, `.agents`, `.claude`, `.gemini`, `.cursor`, `.windsurf`, `.junie`, `.kiro` or `.augment` (the nine as of v3.20.1; earlier releases target a subset) in that clone was a link to a directory elsewhere, the install landed in the link's target. Look there for a `skills` directory of CoalMine skill folders (each holding a `SKILL.md`, with a `.coalmine-manifest.json` beside them), for a `coalmine.json` inside a `coal` directory (the default config, written into a linked `.claude`, `.agents` or `.gemini`), and for a `coalmine-trigger` file inside a `rules` directory. Remove what you did not mean to put there. The skills write-through is measured on 1.0.0 trees from `2aeb8de` (2026-06-05) and applies to every release through v3.20.1; the default config and trigger file were measured on v3.20.1.
